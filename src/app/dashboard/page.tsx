// /workspaces/ncip-app/src/app/dashboard/page.tsx
'use client'

import { useAuth } from '@features/auth/AuthProvider';
import { useRouter } from 'next/navigation';
import { useEffect, useState, useCallback, useRef } from 'react';
import { FaChild, FaBell, FaMapMarkerAlt, FaShieldAlt, FaPlus, FaUser, FaExclamationTriangle, FaSpinner, FaTimes } from 'react-icons/fa';
...
import { UserRole } from '@features/auth/AuthProvider';

export default function DashboardPage() {
  const { user, role, loading, signIn, updateRole, refreshRole } = useAuth();
      ...
      try {
        ...
        if (adminEmails.includes(user.email.toLowerCase())) {
          ...
          if (currentRole !== 'admin') {
            ...
            await refreshRole();
            toast.success('Admin privileges automatically granted');
          }
        }
      } catch (error) {
  ...
  const handleVerifyAdmin = async () => {
    try {
      ...
      await updateRole('admin');
      await refreshRole();
      toast.success('Admin privileges verified successfully');
    } catch (error) {
      console.error("Admin verification error:", error);
  ...
  const fetchChildren = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem('cachedChildren', JSON.stringify(childrenData));
    } catch (error) {
  ...
  const handleAddChild = async () => {
    if (!user?.uid || !newChildName.trim() || !newChildAge) {
      setAlerts([...alerts, 'Please provide both name and age']);
      return;
    }

    const age = parseInt(newChildAge);
    if (isNaN(age) || age < 1 || age > 18) {
      setAlerts([...alerts, 'Please enter a valid age between 1-18']);
      return;
    }
    ...
    try {
      ...
      setAlerts([...alerts, `Successfully added ${newChildName}`]);
      setNewChildName('');
      setNewChildAge('');
      setShowAddChildModal(false);
    } catch (error: any) {
      console.error("Add child error:", error);
      setAlerts([...alerts, `Failed to add child: ${error.message}`]);
    } finally {
      setIsFetching(false);
    }
  };
  ...
  return (
    <RoleGuard allowedRoles={['parent', 'admin', 'ngo', 'community', 'school', 'guardian']}>
      <div className="min-h-screen bg-gray-50 p-4">
        {/* Debugging header - Updated with better admin verification UI */}
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                {user?.email?.includes('unamifoundation.org') && role !== 'admin' && (
                  <button 
        ...
        {role === 'admin' && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {(['parent', 'admin', 'ngo', 'community', 'school', 'guardian'] as UserRole[]).map((r) => (
                <button
                  key={r}
